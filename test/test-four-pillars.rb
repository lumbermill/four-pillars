...
# 時刻不明の場合
fp = FourPillarsLogic.new(["1998","2","27",nil,nil],"f",with_time:true,know_time:false)
puts fp.input
assert(false,fp.setsuiri?)
...
assert(["逆行",8],fp.taiun)

# 時刻不明 + 節入日 + 時刻仮定なし(0:00で算定)
fp = FourPillarsLogic.new(["1964","11","7",nil,nil],"f",with_time:true,know_time:false)
assert(true,fp.setsuiri?)
assert([nil,"庚申","甲戌","甲辰"],fp.kanshi)
assert([nil,"+金","+木","+木"],fp.gogyo_jikkan)
assert([nil,"+金","+土","+土"],fp.gogyo_jyunishi)
assert([nil,57,11,41],fp.kanshi_as_number)
assert(31,fp.zokan_number)
assert([nil,"庚","戊","戊"],fp.zokan)
assert([nil, nil,"偏財","偏財"],fp.tsuhensei)
assert([nil,"比肩","偏印","偏印"],fp.zokan_tsuhensei)
assert([nil,"建禄","衰","養"],fp.jyuniunsei)
assert([nil,11,8,6],fp.jyuniunsei_energy)
assert(25,fp.jyuniunsei_energy.compact.sum)
assert(["子丑","寅卯"],fp.kuubou)
assert(["逆行",10],fp.taiun)
# 時刻不明 + 節入日 + 時刻仮定 (上のテストと日は同じだが月中の干支などが異なる)
fp = FourPillarsLogic.new(["1964","11","7","21","0"],"f",with_time:true,know_time:false)
assert(true,fp.setsuiri?)
assert([nil,"庚申","乙亥","甲辰"],fp.kanshi)
assert([nil,"+金","-木","+木"],fp.gogyo_jikkan)
assert([nil,"+金","-水","+土"],fp.gogyo_jyunishi)
assert([nil,57,12,41],fp.kanshi_as_number)
assert(1,fp.zokan_number)
assert([nil,"戊","甲","乙"],fp.zokan)
assert([nil, nil,"正財","偏財"],fp.tsuhensei)
assert([nil,"偏印","偏財","正財"],fp.zokan_tsuhensei)
assert([nil,"建禄","病","養"],fp.jyuniunsei)
assert([nil,11,4,6],fp.jyuniunsei_energy)
assert(21,fp.jyuniunsei_energy.compact.sum)
assert(["子丑","寅卯"],fp.kuubou)
assert(["逆行",1],fp.taiun)
